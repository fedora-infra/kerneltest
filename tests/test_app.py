...
class KerneltestTests(Modeltests):
    ...
    def test_upload_results_loggedin(self):
        ...
        with user_set(self.app, user):
            ...
            self.assertTrue(
                 (b'<td><input id="test_result" name="test_result" '
                  b'required type="file"></td>') in output.data)
            ...
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<li class="message">Upload successful!</li>' in output.data)
            ...
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<li class="message">Could not parse these results</li>'
                in output.data)
    ...
    def test_upload_results_anonymous(self):
        ...
        self.assertTrue(
            b'<title>405 Method Not Allowed</title>' in output.data)
    ...
    def test_stats(self):
        ...
        self.assertEqual(
            output.data.count(b'<td>3.14.1-200.fc20.x86_64</td>'), 1)
    ...
    def test_index(self):
        ...
        output = self.app.get('/')
        self.assertTrue(
            b"<a href='/kernel/3.14.1-200.fc20.x86_64'>" in output.data)
    ...
    def test_kernel(self):
        ...
        self.assertEqual(
            output.data.decode('utf-8').count('<img src="/static/Denied.png" />'), 4)
    ...
    def test_admin_new_release(self):
        ...
        with user_set(self.app, user):
            ...
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<li class="error">You are not an admin</li>' in output.data)
        ...
        with user_set(self.app, user):
            ...
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<title> New release' in output.data)
            print(output.data)
            self.assertTrue(
                b"<label for=\"releasenum\">Release number"  in output.data)
            ...
            expected_message = {'agent': 'pingou', 'release': {'releasenum': 20, 'support': 'RELEASE'}}
            with fml_testing.mock_sends(ReleaseNewV1(expected_message)):
                output = self.app.post(
                    '/admin/new', data=data, follow_redirects=True)
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<li class="message">Release &#34;20&#34; added</li>'
                in output.data)
            self.assertTrue(b"<a href='/release/20'>" in output.data)
    ...
    def test_admin_edit_release(self):
        ...
        with user_set(self.app, user):
            ...
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<li class="message">No release 21 found</li>'
                in output.data)

            output = self.app.get('/admin/20/edit')
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<h1> Release: 20 </h1>' in output.data)
            self.assertTrue(
                b'<form action="/admin/20/edit" method="POST">'
                in output.data)
            ...
            expected_message = {'agent': 'pingou', 'release': {'releasenum': 21, 'support': 'RAWHIDE'}}
            with fml_testing.mock_sends(ReleaseEditV1(expected_message)):
                output = self.app.post(
                    '/admin/20/edit', data=data, follow_redirects=True)
            self.assertEqual(output.status_code, 200)
            self.assertTrue(
                b'<li class="message">Release &#34;21&#34; updated</li>'
                in output.data)
